...
*** VPC default
By default, a "default subnet" of your VPC is actually a public subnet, because
the main route table sends the subnet's traffic that is destined for the
internet to the internet gateway. You can make a default subnet into a private
subnet by removing the route from the destination 0.0.0.0/0 to the internet
gateway.
...
*** Classic ELB
Cross zone balancing takes into account each machine in all your zones in your VPC.
If you dont have that enabled, the elb will just alternate between the zones.
   aka elb takes a request, it will send it to aza.
       elb takes another request, it will send it to azb
       elb takes another request, it will send it to aza
       elb takes another request, it will send it to azb
       etc
** How do enable cross zone load balancing in the ALB
** Cross Account AMI
...
data that requires frequent updates, such as the system drive for an instance or
storage for a database application. You can also use them for
...
- When you create an EBS volume in an Availability Zone, it is automatically
  replicated within that zone to prevent data loss due to a failure of any
  single hardware component.
- An EBS volume can only be attached to one EC2 instance at a time.
- After you create a volume, you can attach it to any EC2 instance in the same
  Availability Zone
- An EBS volume is off-instance storage that can persist independently from the
  life of an instance. You can specify not to terminate the EBS volume when you
  terminate the EC2 instance during instance creation.
- EBS volumes support live configuration changes while in production which means
  that you can modify the volume type, volume size, and IOPS capacity without
  service interruptions.
- Amazon EBS encryption uses 256-bit Advanced Encryption Standard algorithms
  (AES-256)
...
- traffic to reserved IP address for VPC router is not monitored
* Terms
...
You are helping out a new DevOps Engineer to design her first architecture in
AWS. She is planning to develop a highly available and fault-tolerant
architecture which is composed of an Elastic Load Balancer and an Auto Scaling
group of EC2 instances deployed across multiple Availability Zones. This will be
used by an online accounting application which requires TLS termination
capabilities, path-based routing, host-based routing, and bi-directional
communication channels using WebSockets.

Which is the most suitable type of Elastic Load Balancer that you should
recommend for her to use?



* Best Practices for DDoS Mitigation on AWS
:PROPERTIES:
:youtube:  https://www.youtube.com/watch?v=HnoZS5jj7pk
:slides:   https://www.slideshare.net/AmazonWebServices/best-practices-for-ddos-mitigation-on-aws-91289622
:END:

** 3:57 Types of DDoS Attacks
Slide 7
** 5:03 Why is DDoS a Problem

** 6:56 Traditional Challenges with DDoS Protection
...
** 8:14 Aws Shield Standard

** 10:41 Shield Advanced

** 14:57 10 Best Practices for DDoS Resiliency

** 15:21 Basic Web Application on AWS
** 15:45 Build Scalable...
** 16:07 First Best Practice : Use a Global Distributed System
** 18:37 Second: Make you ELB  Handle demand
** 19:11 Third: Build  a DDoS protected app
** Security 
** 20:28
** 20:48 Fourth
** 21:35 
...
* Deep Dive on Bring Your Own IP - AWS Online Tech Talks
:PROPERTIES:
:youtube:  https://www.youtube.com/watch?v=6G54JaQq5Go
:END:

** 0:20 VPC Foundations
Good rundown on VPC

** 4:36 What is an Elastic IP?

** 5:14 Bring your own IP
...
** 6:36 White listing
- tell your partners who you are at the IP level
  and know this doesnt change
...
** 9:20 How do you use it? 
**  9:30  The address need to be registred (RIR)
** 10:00 Preparing your IP
1.create a ROA to authorize amazon to advertize your address
** 10:41 ROA 
** 11:01 Wher can i get a ROA?
ARIN

** 11:20 Preparing your IP Range
 2.Generate an rsa 2018-bit key pair
...
* General Tips
* 
https://www.reddit.com/r/AWSCertifications/comments/ct7fjk/aws_security_specialist/exjcjj2/
I studied for - and passed - this exam using only A Cloud Guru's materials
(November 2018). I think their content spends the appropriate amount of time on
each of the topics, based on their weight, and the labs are effective. With what
you learned in the SAA course materials, particularly around S3, logs, and IAM,
you should be fine.

That said, my first experience with Linux Academy was with the SysOps
Administrator exam, after finding that the A Cloud Guru content for that exam
was not sufficient at all. I really like Linux Academy's SysOps course, and it
matched the test material very closely. I didn't have any surprises on the exam.
I would be surprised if Linux Academy's Security Specialty exam content misses
the mark.

Test prep aside, I can give you some advice about the test in general. First,
read every word in every question. Read it again. Read the sentences out of
order, which will force your brain to focus on the words in the sentence and not
the idea your brain thinks they are trying to convey. If you have a tendency to
work quickly and try to capture the general concepts of the question (like me),
you will not do well on this exam. This exam is not difficult, it's tricky. A
word can be practically buried in a question, e.g. "An auditor cannot access the
S3 buckets where logs are stored," vs. "An auditor cannot access some of the S3
buckets where logs are stored." The word "some" in the second phrase will
totally change how you answer that question. Work slowly and make sure you
understand each question completely.

Second, the topics you should spend the most time on are:

When an action can be governed by IAM policy, S3 bucket policy, and object
ACL's, how will the request be evaluated? At the time I took the exam,
Organizations and Service Control Policies were not covered. Federation, IAM
roles, and Cognito Logging with CloudTrail and securing those logs What
GuardDuty, Trusted Advisor, Inspector, Config, and Shield do, and what they
don't do. Secondarily, know which services are proactive vs. reactive, and
how/if they respond to events. Be able to read S3 bucket policies and IAM
policies, and understand what the effects of a policy they give you will be.
Generally, the best exam tip for any AWS exam is to not start out by looking for
right answers, but for ones that are definitely wrong. Services that don't
actually exist, services that don't work the way the potential answer suggests,
or scenarios that aren't possible... eliminating those will make this test
easier. On difficult questions, you'l be left with one or two options, and
usually you'll just have to decide which option is more efficient,
cost-effective, or practical.

All-in-all, I found the SA-A exam to be more challenging than this one. I would
also recommend going to SysOps Admin (if that's your bag) after this, because
75% of that exam overlaps with Security and SA-A.
